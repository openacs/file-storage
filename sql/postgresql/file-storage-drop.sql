...
-- JS: trying to delete the root folder will raise an exception.  Also, acs_object__delete 
-- JS: is the wrong way to delete the items in CR. To delete a folder, for example, we must
-- JS: call content_folder__delete, which in turn calls  acs_object__delete only through
-- JS: content_item__delete and only after cleaning up stuff).  So if the package is deleted but 
