...
Sabino</a> for OpenACS.
<h3>I. Essentials</h3><ul>
...
</ul><h3>II. Introduction</h3><p>We have our own file-storage application because we want all
...
</ul><p>We want something that is relatively secure, and can be extended
and maintained by any ArsDigita programmer, <i>i.e.</i>, something
that requires only AOLserver Tcl and Oracle skills.</p><p>In ACS 4, File Storage can be implemented on top of the Content
...
Content Repository.</p><h3>III. Historical Considerations</h3><p>File Storage was created to provide a mechanism for
...
having to get bogged down by sifting through multiple versions.</p><h3>IV. Competitive Analysis</h3><p>Why is a file-storage application useful?</p><p>If you simply give everyone FTP access to a Web-accessible
...
server.</p><p>The File Storage application is not a web-based file system, and
...
of a given type, or searching through specified file types).</p><h3>V. Design Tradeoffs</h3><h4>Folder Permissions</h4><p>Previous versions of File Storage have not included folder
...
permissions.</p><p>Note that this can create some funny results. For example, a
...
moving and copying files to look odd or misleading.</p><h4>Deletion of Files</h4><p>Previous versions of File Storage allowed only administrators to
...
asks to delete a version or a file, we really delete it.</p><h4>Use of Content Repository</h4><p>Basing this system on the Content Repository provides a wealth
of useful functionality for File Storage with no additional
development costs. However, it may also constrain the system
somewhat.</p><p>The Content Repository's datamodel has been extended to include
...
database..</p><p>The content_revision is subtyped as a "file-storage-item" to
allow site-wide search to distinguish file storage objects in its
search results. This feature is not implemented yet, however.</p><h4>Permissions Design</h4><p>Permissions were chosen to make as much use as possible of the
...
context.</p><table width="100%" border="2">
<tr>
...
</table><p>Some notes: the admin privilege implies the read, write and
...
write permission does not have any meaning for versions.</p><h3>VI. API</h3><p>For the most part, File Storage will provide wrappers to the
Content Repository APIs.</p><h4>PL/SQL API</h4><p>File Storage provides public PL/SQL APIs either as wrappers to
...
its objects.</p><p>The main objects of File Storage are "folders" and "files". A
...
thus folders are stored "as is" in the Content Repository.</p><p>"Files", however, can cause some confusion when stored in the
...
"title".</p><p>Each version of a file is stored as a revision in cr_revisions
...
filename of the version uploaded into the Content Repository.</p><p>Meta-data about a version of a file stored in Content Repository
are the size of the version (stored in cr_revisions.content_length)
and version notes (stored in cr_revisions.description).</p><p>There are two internal PL/SQL functions that do not call the
...
</p><h4>Tcl API</h4><table width="100%"><tr><td bgcolor="#E4E4E4">
...
</td></tr></table><table width="100%"><tr><td bgcolor="#E4E4E4">
...
</td></tr></table><h3>VII. Data Model Discussion</h3><p>File Storage uses only the Content Repository data model. There
...
Content Repository.</p><p>Inserting a row into the table fs_root_folders occurs the first
...
identifier.</p><p>There is an "on delete cascade" constraint imposed on the
...
from which all content is referenced!</p><p>The solution is (hopefully) more elegant: an "before on delete"
...
deleting them from the database.</p><h3>VIII. User Interface</h3><p>The user interface attempts to replicate the file system
...
appropriate.</p><h3>IX. Configuration/Parameters</h3><p>There are two configuration parameters in this version of File
...
files in the fiesystem.</p><p>When a file is stored in the Content Repository, it first
...
changed.</p><p>All of the other parameters in previous versions have been made
obsolete by ACS 4 features like site-nodes and templating.</p><h3>X. Future Improvements/Areas of Likely Change</h3><ul>
...
</ul><h3>XI. Authors</h3><ul><li>System creator:<br>
</li></ul><blockquote>3.x : <a href="mailto:dh\@caltech.edu">David Hill</a>
and <a href="http://aure.com/">Aurelius Prochazka</a><br>
4.x : <a href="mailto:kevin\@arsdigita.com">Kevin
Scaldeferri</a>
</blockquote><ul><li>System owner</li></ul><blockquote><a href="mailto:kevin\@arsdigita.com">Kevin
Scaldeferri</a></blockquote><ul><li>Documentation author</li></ul><blockquote><a href="mailto:kevin\@arsdigita.com">Kevin
Scaldeferri</a></blockquote><h3>XII. Revision History</h3><table cellpadding="2" cellspacing="2" width="90%" bgcolor="#EFEFEF">
<tr bgcolor="#E0E0E0">
