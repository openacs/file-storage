...
set context [fs_context_bar_list -final \
                [_ file-storage.Add_pretty_name \
                    [list pretty_name $pretty_name]] $folder_id]
...
ad_form -extend -form {
    ...
    set item_id [content::extlink::new \
                    -url $url \
                    -label $title \
                    -description $description \
                    -parent_id $folder_id]
    #
    # Analogous as for files (see file-add-2) we know the user has write
    # permission to this folder,  but they may not have admin privileges.
    # They should always be able to admin their own url (item) by default, so
    # they can delete it, control who can read it, etc.
    #
    if { [string is false [permission::permission_p \
                                -party_id $user_id \
                                -object_id $folder_id \
                                -privilege admin]]
    } {
    ...
    if { [parameter::get -parameter CategoriesP \
